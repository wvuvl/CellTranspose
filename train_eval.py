    ...
    for e in range(1, n_epochs + 1):
        ...
        for batch in range(len_data):
            ...
            target_sample_labels = as_tensor(
                [LabelsToFlows()(target_sample_labels[i].numpy()) for i in range(len(target_sample_labels))])
            ...
            # source_batch_labels = source_batch_labels.float().to(device)
                ...
                target_sample_patch_data = target_sample_data[patch_ind:patch_ind + patch_per_batch].float().to(device)
                target_sample_patch_labels = target_sample_labels[patch_ind:patch_ind + patch_per_batch].float().to(device)
